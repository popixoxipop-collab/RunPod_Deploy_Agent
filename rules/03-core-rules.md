...
## R12. 대형 모델 로딩은 정적 분석 먼저 (2026-04-11 추가)
...
4. Hook 함수에서 텐서 CPU 연산 확인
5. 사용 라이브러리 버전 + 모델 조합의 known issue 검색

**근거**: 크래시 → 수정 → 재시도 반복은 $30~$100 단위 손실.
사용자가 "왜 계속 크래시가 나서야 고치냐고 사전에 고치라고 내가 기회를 몇 번이나 줬는데"
라고 항의하기 전에 전부 체크할 것.

---

## R13. 장기 작업 on-demand 강제 (2026-04-11 추가)
...
## R14. Network Volume 삭제 금지 (2026-04-09 추가)

- `deleteNetworkVolume` GraphQL mutation 호출 금지
- MCP `delete-network-volume` 도구 호출 금지
- 사용자가 **명시적으로** "삭제해"라고 말한 경우에만 허용
- DC 변경 필요 시: 새 볼륨 생성 → 데이터 복사 → 사용자 확인 → 구 볼륨 삭제
